...
exports.getHome = async (req, res) => {
  try {
    ...
    // console.log(tests);
    const usertests = await User.findById(res.locals.id);
    let userTest = new Array();
    usertests.tests.forEach(test => {
      userTest.push(test.test);
    })
    console.log(userTest);
    let refineTests = new Array();
    tests.forEach(test => {
      check = 0;
      for (let i = 0; i <= userTest.length; i++) {
        if (test._id.equals(userTest[i])) {
          check = 1;
          console.log("coming here");
          refineTests.push({ test, hasgiven: true });
        }

      }
      if (check != 1) {
        refineTests.push({ test, hasgiven: false });
      }
    });
    console.log(refineTests)
    res.render("home", {
      articles: articles,
      tests: refineTests,
...
exports.get_termsAndConditions = (req,res)=>{
